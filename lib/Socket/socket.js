...
const makeSocket = (config) => {
    var _a, _b;
    const { waWebSocketUrl, connectTimeoutMs, logger, keepAliveIntervalMs, browser, auth: authState, printQRInTerminal, defaultQueryTimeoutMs, transactionOpts, qrTimeout, makeSignalRepository, autoFetchVersion, optimizeConnection, } = config;
    // baileys-premod: Auto-fetch latest WA Web version
    if (autoFetchVersion !== false) {
        (0, Utils_1.fetchLatestWaWebVersion)(config.options).then(({ version, isLatest }) => {
            if (isLatest) {
                config.version = version;
                logger.info({ version }, 'auto-fetched latest WA Web version');
            }
            else {
                logger.warn('failed to fetch latest WA version, using default');
            }
        }).catch(err => {
            logger.warn({ err }, 'failed to auto-fetch WA version');
        });
    }
    const url = typeof waWebSocketUrl === 'string' ? new url_1.URL(waWebSocketUrl) : waWebSocketUrl;
    // baileys-premod: Mobile API support restored
    const isMobile = config.mobile || url.protocol === 'tcp:';
    if (url.protocol === 'wss' && ((_a = authState === null || authState === void 0 ? void 0 : authState.creds) === null || _a === void 0 ? void 0 : _a.routingInfo)) {
        url.searchParams.append('ED', authState.creds.routingInfo.toString('base64url'));
    }
    const ws = isMobile
        ? new Client_1.TCPSocketClient(url, config)
    ...
    const getAvailablePreKeysOnServer = async () => {
        ...
        if (!result) {
            logger.warn('getAvailablePreKeysOnServer: query timed out, returning 0');
            return 0;
        }
    ...
    const uploadPreKeys = async (count = Defaults_1.INITIAL_PREKEY_COUNT, retryCount = 0) => {
        ...
        // Prevent multiple concurrent uploads — set the promise BEFORE starting work
        if (uploadPreKeysPromise) {
            logger.debug('Pre-key upload already in progress, waiting for completion');
            await uploadPreKeysPromise;
            // After waiting, check if we still need to upload
            if (Date.now() - lastUploadTime < Defaults_1.MIN_UPLOAD_INTERVAL) {
                return;
            }
        }
        ...
        uploadPreKeysPromise = Promise.race([
            ...
            new Promise((_, reject) => {
                timeoutId = setTimeout(() => reject(new boom_1.Boom('Pre-key upload timeout', { statusCode: 408 })), Defaults_1.UPLOAD_TIMEOUT);
            })
    ...
    const requestPairingCode = async (phoneNumber, pairCode) => {
        ...
        ev.emit('creds.update', { pairingCode: authState.creds.pairingCode, me: { id: authState.creds.me.id, name: '~' } });
        await sendNode({
            tag: 'iq',
            attrs: {
                to: WABinary_1.S_WHATSAPP_NET,
