...
import { WebSocketClient, TCPSocketClient } from './Client'
...
export const makeSocket = (config: SocketConfig) => {
        ...
        if(autoFetchVersion !== false) {
                fetchLatestWaWebVersion(config.options).then(
                        ({ version, isLatest }) => {
                                if(isLatest) {
        ...
        const ws = isMobile
                ? new TCPSocketClient(url, config)
        ...
        const uploadPreKeys = async(count = INITIAL_PREKEY_COUNT, retryCount = 0) => {
                ...
                if(uploadPreKeysPromise) {
                        ...
                        if(Date.now() - lastUploadTime < MIN_UPLOAD_INTERVAL) {
                                return
                        }
                }
