...
export const generateProfilePicture = async(mediaUpload: WAMediaUpload) => {
        ...
        if('sharp' in lib && typeof lib.sharp?.default === 'function') {
                const sharpImg = lib.sharp!.default(bufferOrFilePath as any)
                const metadata = await sharpImg.metadata()
                const cropped = metadata.width! > metadata.height!
                        ? sharpImg.resize(550, null)
                        : sharpImg.resize(null, 650)
                img = cropped.jpeg({ quality: 100 }).toBuffer()
        } else if('jimp' in lib && typeof lib.jimp?.read === 'function') {
                const jimp = await lib.jimp.read(bufferOrFilePath as any)
                const cropped = jimp.getWidth() > jimp.getHeight() ? jimp.resize(550, -1) : jimp.resize(-1, 650)
                img = cropped
                        .quality(100)
                        .getBufferAsync(lib.jimp.MIME_JPEG)
        } else {
                throw new Boom('No image processing library available. Install jimp or sharp.', { statusCode: 400 })
        }
